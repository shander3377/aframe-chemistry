	...
	init: async function () {
      ...
      elementsArray.push({
		...
		this.el.addEventListener("markerLost", () => {
			var elementName = this.el.getAttribute("element_name");
			var index = elementsArray.findIndex(
				(x) => x.element_name === elementName
			);
			if (index > -1) {
...
console.log(length)
      if (length === 3) {
        ...
        if (distance1 < 1.25 && distance2<2.5) {
          if (compound !== undefined) {
	...
	countOccurence: function (array, val) {
		return array.reduce((a, v) => (v.element_name === val ? a++ : a), 0);
	},
	...
	showCompound: function (compound,barcodeValue) {
		//Hide elements
